...
public class Test {
	public static void main(String[] args) {
		...
		// Fluent interfaces
		System.out.println(service.path("gqgk").path("gatekeeper").accept(
				MediaType.TEXT_PLAIN).get(ClientResponse.class).toString());
		System.out.println(service.path("gqgk").path("hello").accept(
				MediaType.TEXT_PLAIN).get(String.class).toString());

		System.out.println("-------text plain over--------------\n");
		
		// Get app xml text
		System.out.println(service.path("rest").path("gatekeeper").accept(
	...
	private static URI getBaseURI() {
		return UriBuilder.fromUri(
				"http://localhost:8080/RESTProject/").build();
	}
