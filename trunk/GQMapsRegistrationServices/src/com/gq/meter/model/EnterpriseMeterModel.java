...
public class EnterpriseMeterModel {
    ...
    public List<EnterpriseMeter> getProtocol(String entpId) throws Exception {
        ...
        try {
            ...
            String hql = "SELECT DISTINCT(protocolId), count(protocolId) FROM EnterpriseMeter where enterprise_id = :ENT_ID group by protocolId";
            Query query = session.createQuery(hql);
            query.setParameter("ENT_ID", entpId);
            List<Object[]> entMeterResult = query.list();

            List<EnterpriseMeter> meterList = new ArrayList<EnterpriseMeter>();
