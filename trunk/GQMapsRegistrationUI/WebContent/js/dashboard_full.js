...
//loading top 5 most used assets

function loadMostAssets()
{
	
	var vType = "GET";						
	
	var vUrl = $.jStorage.get("jsDBUrl") + "customerservices/mostConsumedAssets";	
		...
		({
			...
			success:function(json)
			{
				...
				if($.jStorage.get("jsMeters") != 0)
				{
					...
					jQuery.each(json, function(i, v) 
					{
						vMostAssets = vMostAssets + "<tr><td class = 'td-font'><span class='span-color'>" + v["name"] + "</span></td><td class = 'td-font'><span class='span-color'>" + v["loadAvg"] + "</span></td></tr>";	
				...
				{
					vMostAssets = vMostAssets + "N/A";
				}				
					
				$("#mostAssets").append(vMostAssets);
				$("#mostAssets1").append(vMostAssets);							  
			},
			error:function(json)
			{
				alert("Error from loading most loaded assets: " + json.status + " " + json.responseText);
			} 
		});	
	}
	else
...
function loadLeastAssets()
{
	
	var vType = "GET";						
	
	var vUrl = $.jStorage.get("jsDBUrl") + "customerservices/leastConsumedAssets";	
		...
		({
			...
			success:function(json)
			{
				...
				if($.jStorage.get("jsMeters") != 0)
				{
					...
					jQuery.each(json, function(i, v) 
					{
						vLeastAssets = vLeastAssets + "<tr><td class = 'td-font'><span class='span-color'>" + v["name"] + "</span></td><td class = 'td-font'><span class='span-color'>" + v["loadAvg"] + "</span></td></tr>";	
				...
				{
					vLeastAssets = vLeastAssets + "N/A";
				}				

				$("#leastAssets").append(vLeastAssets);							  
			},
			error:function(json)
			{
				alert("Error from loading least loaded assets: " + json.status + " " + json.responseText);
			} 
		});	
	}
	else
