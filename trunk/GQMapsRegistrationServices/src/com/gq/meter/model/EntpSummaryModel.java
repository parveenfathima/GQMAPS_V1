...
public class EntpSummaryModel
{
	...
	public List<EntpSummary> getEntpSummaryList() throws Exception
	{
		...
		try
		{
			...
			String hql = "select e.sid, e.enterpriseId, e.blCd, e.eName, e.phone, e.email, e.userId, e.passwd, e.secQtn1, e.ans1, e.secQtn2, e.ans2, e.storeFwd, e.fwdUrl, e.noOfEmpl, e.entSqft, e.entAssetCount, e.dcSqft, e.dcAssetCount, e.dcUsePctg, e.dcTemp, e.regCmplt, e.active, e.comments, e.creDttm, (select count(em.meterId) from EnterpriseMeter em where e.enterpriseId = em.enterpriseId), (select g.expDttm from GateKeeper g where e.enterpriseId = g.enterpriseId) from Enterprise e";
			Query query = session.createQuery(hql);
			...
			for (Object[] list : summaryResult)
			{
				...
				entpSummary.setmCount((Long) list[25]);
				entpSummary.setExpDttm((Date) list[26]);
