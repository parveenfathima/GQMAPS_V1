...
//		ClientResponse response = service.path("gqm-gk").path("gatekeeper").path(todo.getId()).accept(MediaType.APPLICATION_XML).put(ClientResponse.class, todo);
...
//		System.out.println("-------------------------------------------------------------------");
		...
		ClientResponse response = service.path("gqm-gk").path("gatekeeper").type(MediaType.APPLICATION_JSON)
...
//				MediaType.APPLICATION_XML).get(String.class));
	...
	private static URI getBaseURI() {
		return UriBuilder.fromUri(
				"http://localhost:8080/GQGatekeeper").build();
	}
}
